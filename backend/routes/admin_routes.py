...
def get_admin_reports(telecaller_id: int = None, start_date: str = None, end_date: str = None, prospect_type: str = None):
    ...
    def _pt_clause(alias, p_list):
        ...
        if prospect_type:
            ...
            if prospect_type == 'college_contact':
                ...
                # Exclude both short_term_course and legacy edii values for backward compatibility
                return f" AND {table}.prospect_type IS DISTINCT FROM 'short_term_course' AND {table}.prospect_type IS DISTINCT FROM 'edii' AND ({table}.lead_source IS NOT NULL AND jsonb_array_length({table}.lead_source) > 0 OR {table}.lead_type IS NOT NULL AND jsonb_array_length({table}.lead_type) > 0) "

            if prospect_type == 'short_term_course':

                # Support both 'short_term_course' and legacy 'edii' values for backward compatibility
                return f" AND ({table}.prospect_type = 'short_term_course' OR {table}.prospect_type = 'edii') "
            ...
            if prospect_type == 'student_admission':
                ...
                # Exclude both short_term_course and legacy edii values for backward compatibility
                return f" AND {table}.prospect_type IS DISTINCT FROM 'short_term_course' AND {table}.prospect_type IS DISTINCT FROM 'edii' AND NOT ({table}.lead_source IS NOT NULL AND jsonb_array_length({table}.lead_source) > 0 OR {table}.lead_type IS NOT NULL AND jsonb_array_length({table}.lead_type) > 0) "
    ...
    if prospect_type in ('college_contact', 'short_term_course'):
    ...
    telecaller_performance = execute_query(f"""
        ...
        SELECT
            ...
            COUNT(cl.id) FILTER (WHERE cl.status_after_call = 'Interested') as "shortTermCourseInterestedCount",

            COUNT(cl.id) FILTER (WHERE cl.status_after_call IN ('Interested Followup', 'Interested-Followup')) as "shortTermCourseInterestedFollowupCount",

            COUNT(cl.id) FILTER (WHERE cl.status_after_call = 'Qualified') as "shortTermCourseQualifiedCount",
