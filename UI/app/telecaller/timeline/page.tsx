...
import { prospectsApi, type ActivityFeedItem, type ActivityFeedResponse } from "@/lib/api-client"
import { useAuth } from "@/lib/auth-context"
import { formatISTDate, formatISTTime, cn } from "@/lib/utils"
import { PageSkeleton } from "@/components/ui/loading-skeletons"

export default function TelecallerActivityTimelinePage() {
  const { user } = useAuth()
  ...
  const fetchFeed = useCallback(async (silent = false) => {
    ...
    try {
      ...
      console.error("Failed to fetch telecaller activities feed:", err)
    } finally {
      setIsLoading(false)
      setIsRefreshing(false)
    }
  }, [user, activityType, onlyConverted, search, calculatedStartDate, calculatedEndDate])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
            ...
            My Activity & Conversion Timeline
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Your personal activity feed showing lead conversions, payments recorded, call updates, and student milestones.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-border bg-card shadow-xs">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase">My Activities</p>
        ...
        <Card className="border-border bg-card shadow-xs">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-emerald-700 uppercase">My Conversions</p>
        ...
        <CardContent className="p-4 space-y-3">
          <div className="flex flex-col md:flex-row gap-3 items-center">
            ...
            <div className="relative flex-1 w-full">
              ...
              <Input
                placeholder="Search student name, phone, lead ID, or details..."
            ...
            <Select value={activityType} onValueChange={setActivityType}>
              <SelectTrigger className="w-full md:w-[180px] h-9 bg-background text-sm">
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed border-2 bg-card p-12 text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            No activity records match your current filters. Try changing your search query or selecting a different date range.
          </p>
          <Button
            variant="outline"
            size="sm"
              ...
              <div className="space-y-3 pl-2 sm:pl-4 relative before:absolute before:left-[19px] sm:before:left-[27px] before:top-3 before:bottom-3 before:w-0.5 before:bg-border">
                {group.items.map((item) => {
                  const detailHref = item.converted_enquiry_id
                    ? `/telecaller/converted-enquiries/${item.converted_enquiry_id}`
                    : `/telecaller/qualified-leads`
