...
for idx, line in enumerate(lines):
    if start is None and line.strip() == '// Filter logs':
        start = idx
    if start is not None and line.strip().startswith('const paginatedLogs = useMemo(() => {'):
        end = idx
        break
if start is None or end is None:
    raise RuntimeError(f'start={start} end={end} not found')
replacement = '''  // Filter logs
  ...
  const getLogCourses = (log: CallLog, prospect?: Prospect) => {
    ...
    return prospect.course_interest
      .split(",")
      .map((c: string) => c.trim())
      .filter(Boolean)
  }
      ...
      return [
        {
          ...
          displayCourse:
            courses[0] ||
            (log.course_interest || prospect?.course_interest || "").trim() ||
            "",
...
path.write_text("\n".join(lines) + "\n", encoding='utf-8')
print('patched from', start, 'to', end)
